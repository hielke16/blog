...
// load the SplAutoloader interface
...
class StandardAutoloader implements SplAutoloader
{
    ...
    const AUTOREGISTER_WEBWIJS  = 'autoregister_webwijs';
     ...
     * array(
     *     'namespaces' => array(
     *         'Webwijs'     => '/path/to/Webwijs/library',
    ...
    public function setOptions($options)
    {
        ...
        foreach ($options as $type => $pairs) {
            switch ($type) {
                case self::AUTOREGISTER_WEBWIJS:
                    if ($pairs) {
                        $this->registerNamespace('Webwijs', dirname(__DIR__));
                    }
                    break;
                case self::LOAD_NS:
